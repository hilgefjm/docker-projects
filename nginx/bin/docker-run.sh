...
for tmp in $(docker ps -aq --filter='image='$IMAGE); do
	INSTANCE=$(($INSTANCE + 1))
	docker rename $(python getName.py $(docker inspect $tmp)) 'redis-0'$INSTANCE
...
for i in $(docker ps -aq --filter='image='$IMAGE); do
	INSTANCE=$(($INSTANCE + 1))
	docker rename $(python getName.py $(docker inspect $i)) 'redis-'$INSTANCE
done
...
INSTANCE=$(($INSTANCE + 1))
docker run -d --name='redis-'$INSTANCE -p=$INSTANCE_PORT:$PORT $IMAGE /usr/bin/redis-server
